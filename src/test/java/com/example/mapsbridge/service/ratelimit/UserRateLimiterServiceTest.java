...
public class UserRateLimiterServiceTest {
    ...
    @BeforeEach
    public void setUp() {
        userRateLimiterService = new UserRateLimiterService(rateLimiterRegistry);
    }

    @AfterEach
    public void tearDown() {
        // Clear the logging context after each test
    ...
    public void testCheckRateLimit_Success() {
        ...
        LoggingContext.setIpAddress(testIp);

        // When & Then
        assertDoesNotThrow(() -> userRateLimiterService.checkIpRateLimit());
    ...
    public void testCheckRateLimitForEmail_Success() {
        ...
        // When & Then
        assertDoesNotThrow(() -> userRateLimiterService.checkRateLimitForEmail(testEmail));
    ...
    public void testCheckRateLimitForEmail_ExceedsLimit() {
        ...
        EmailRateLimitExceededException exception = assertThrows(
                ...
                () -> userRateLimiterService.checkRateLimitForEmail(testEmail)
        );
    ...
    public void testCleanupOldIpRateLimiters() {
        ...
        ReflectionTestUtils.setField(userRateLimiterService, "ipRateLimiters", ipRateLimiters);
        ReflectionTestUtils.setField(userRateLimiterService, "ipLastAccessTimes", ipLastAccessTimes);

        // Execute cleanup with 24 hours threshold
        int removedCount = userRateLimiterService.cleanupOldIpRateLimiters(24);
    ...
    public void testCleanupOldEmailRateLimiters() {
        ...
        // Use reflection to set the private fields
        ReflectionTestUtils.setField(userRateLimiterService, "emailRateLimiters", emailRateLimiters);
        ReflectionTestUtils.setField(userRateLimiterService, "emailLastAccessTimes", emailLastAccessTimes);

        // Execute cleanup with 24 hours threshold
        int removedCount = userRateLimiterService.cleanupOldEmailRateLimiters(24);
    ...
    public void testUpdatesLastAccessTimeForIp() {
        ...
        Map<String, LocalDateTime> ipLastAccessTimes = (Map<String, LocalDateTime>)
                ReflectionTestUtils.getField(userRateLimiterService, "ipLastAccessTimes");
    ...
    public void testUpdatesLastAccessTimeForEmail() {
        ...
        Map<String, LocalDateTime> emailLastAccessTimes = (Map<String, LocalDateTime>)
                ReflectionTestUtils.getField(userRateLimiterService, "emailLastAccessTimes");
    ...
    @Test
    public void testCheckRateLimitForChatId_ThrowsUnsupportedOperationException() {
        // Given
        ...
        UnsupportedOperationException exception = assertThrows(
                ...
                () -> userRateLimiterService.checkRateLimitForChatId(testChatId)
        );

        assertEquals("Chat ID-based rate limiting is not supported for user sign-ups", exception.getMessage());

        // Verify that the rate limiter was not called
        verify(rateLimiterRegistry, never()).rateLimiter(anyString(), anyString());
        verify(rateLimiter, never()).acquirePermission();
    }

    // Tests for checkRateLimitForChatId have been consolidated into a single test
    // testCheckRateLimitForChatId_ThrowsUnsupportedOperationException
    // since the method now always throws UnsupportedOperationException
    ...
    public void testCleanupOldChatIdRateLimiters() {
        ...
        // Use reflection to set the private fields
        ReflectionTestUtils.setField(userRateLimiterService, "chatIdRateLimiters", chatIdRateLimiters);
        ReflectionTestUtils.setField(userRateLimiterService, "chatIdLastAccessTimes", chatIdLastAccessTimes);

        // Execute cleanup with 24 hours threshold
        int removedCount = userRateLimiterService.cleanupOldChatIdRateLimiters(24);
    ...
    // Test for updating last access time for chat ID has been removed
    // since checkRateLimitForChatId now throws UnsupportedOperationException
}